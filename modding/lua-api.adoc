...
_Game Rules_ and _Game Settings_ are discussed above in sections 2.3 and 2.4 above.
...
See sections 6.2.49 to 6.2.52 below for calls relating to the volunteer feature.
...
There is currently no way to manually set a player element via lua script, but see Section 5.4.4 below for assigning default elements on server join.
...
Some limited manipulation of player patches may be possible via the inventory system (see Section 4 below).
...
You will recall the default set of round stages listed in Section 2.1 above.
...
See Section 4 and Section 6.9 below for more information.
...
In response the game mode is able to return a custom loadout to apply to the player (as might be set up, for example, by `+inventory.CreateLoadoutFromTable()+` – see Section 6.9.6 below). The function either returns the loadout name to apply, or `+nil+` to proceed with the default loadout.
...
The loadout is passed as `+USERDATA+` encapsulating an array of JSON objects corresponding to the '`monolithic`' JSON kit list (see Section 4.1.2 above). This is a good place for pistols only mutators and suchlike to remove things.
...
See section 2.1 above for more information on round stages.
...
A player state can be obtained from player.GetPlayerState() (see Section 6.5.7 below).
...
As also explained in section 2.6 above, the objective description is looked up in the string table using the format `+“objective_”+` + [.var]#Name#. The Uplink game mode, for example, has objectives DefendObjective and CaptureObjective, which are stored as follows in the `+Uplink.csv+` string table:
...
See section 2.8 for an illustration of the team score table format (see also the DTAS game mode). This must be done when a game mode initialises, and before any team scores are awarded.
...
See section 2.7 for an illustration of the player score table format (see also the DTAS game mode). This must be done when a game mode initialises, and before any player scores are awarded.
...
See section 6.5.18 for a list of possible ready statuses.
...
See gamemode.BroadcastGameMessage (see 6.2.9 above) for more info on the [.var]#Type# and [.var]#Duration# parameters.
...
This function awards the specified score type [.var]#ScoreName# a multiple [.var]#ScoreMultiple# of times (unless the relevant score type has a OneOff property of `+true+`). Player scores must have been set up via gamemode.SetPlayerScoreTypes() first (see 6.2.35 above).
...
See also gamemode.SpawnEffectAtLocation() (see Section 6.2.54).
...
The function attempts to load the file with name [.var]#KitFileName# from the GroundBranch/CustomKit folder, and returns the result as a lua table encoding the stored kit, in the same format as the returned table in GetPlayerLoadoutAsTable (Section 6.9.1 above). If `+SplitItemField+` is true, then `+Item+` fields will be returned as separate `+ItemType+` and `+ItemValue+` fields, as above.
...
This function carries out the reverse process of GetPlayerLoadoutAsTable() (see Section 6.9.1 above) and creates a JSON version of the kit list in the [.var]#LoadoutTable# lua table, and then stores that as a new or amended loadout for the player [.var]#Player#, using loadout name [.var]#LoadoutName#. The value of [.var]#SplitItemField# should be the same as that used originally to generate the lua table.
...
The next two functions operate on Loadout Reference Objects which are USERDATA references that are unique parameters for the OnPreLoadoutChanged() function described in Section 5.4.6 above.
